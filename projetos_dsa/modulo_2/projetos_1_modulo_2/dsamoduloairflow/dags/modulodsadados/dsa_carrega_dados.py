# Projeto 1 - LLMOps - Deploy e Operacionalização de Aplicação de IA Generativa com LLM e RAG
...
from modulodsadados.dsaconnection import postgre_connection
from modulodsadados.dsa_get_dados import dsa_extrai_dados

# Função para gerar um ID único para cada documento com base no conteúdo
def dsa_gera_id_documento(doc):
...
# Função para criar a tabela 'dsa_documentos' no banco de dados
def dsa_cria_tabela():
...
# Função para inserir dados JSON na tabela 'dsa_documentos'
def dsa_insere_dados_json():

    # Inicializa uma lista para armazenar os dados a serem inseridos
    allData = []
    
    # Carrega os dados JSONL do arquivo especificado
    dsaDados = orjsonl.load(f"{os.getcwd()}/dags/dados/dataset1.jsonl")
    ...
    insert_query = """
    INSERT INTO dsa_documentos (doc_id, question, answer) VALUES (%s, %s, %s)
    """

    # Processa cada entrada no JSON para os primeiros 25 registros
    for data in dsaDados[0:25]:
        ...
        docId = dsa_gera_id_documento(data)
    ...
    try:
        ...
        args = ','.join(cur.mogrify("(%s,%s,%s)", i).decode('utf-8')
                        for i in allData)
        insert_query = "INSERT INTO dsa_documentos (doc_id, question, answer) VALUES" + (args)
...
# Função para inserir dados CSV na tabela 'dsa_documentos'
def dsa_insere_dados_csv():
    ...
    dsaDados = pd.read_csv(f"{os.getcwd()}/dags/dados/dataset2.csv")

    # Processa as primeiras 25 linhas do CSV
    for index, row in dsaDados.head(25).iterrows():
...
def dsa_cria_indice():
    ...
    # Itera sobre os documentos e indexa cada um no Elasticsearch (sistema de RAG)
    for doc in data:
        try:
    ...
    return "DSA - Dados Carregados com Sucesso"
